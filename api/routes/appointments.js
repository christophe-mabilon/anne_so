...
const sendEmailIfNotSent = async (appointment, type) => {
	...
	if (!column) {
		logger.error("Type d'email non reconnu :", type);
		return;
	}
	...
	try {
		...
		if (result[0][column]) {
			logger.info(`Email déjà envoyé pour ${type}`);
			return;
		}
		...
		logger.error(`Erreur lors de l'envoi de l'email (${type}):`, error.message);
	}
};
	...
	try {
		...
		const slots = isSaturday
			...
			: [
					...
					{
						start: moment.tz(
							`${formattedDate} ${afternoon_open}`,
							"Europe/Paris"
						),
						end: moment.tz(
							`${formattedDate} ${afternoon_close}`,
							"Europe/Paris"
						),
					},
			  ];
		...
		if (availabilities.length === 0) {
			...
			res
				.status(200)
				.json({ message: "Disponibilités trouvées", availabilities });
		}
	} catch (err) {
		logger.error("Erreur lors de la récupération des disponibilités :", err);
...
router.post("/", async (req, res) => {
	try {
		...
		if (
			...
			return res
				.status(400)
				.send("Tous les champs requis doivent être remplis");
		}
      ...
      `;
		...
		try {
			...
			logger.error("Erreur lors de la confirmation de rendez-vous :", error);
		}
	} catch (err) {
...
router.get("/", async (req, res) => {
	try {
		...
		logger.error("Erreur lors de la récupération des rendez-vous :", err);
		res.status(500).send("Erreur serveur");
	}
});
...
router.get("/:id", async (req, res) => {
	try {
		...
		logger.error("Erreur lors de la récupération du rendez-vous :", err);
		res.status(500).send("Erreur serveur");
	}
});
...
router.delete("/:id", async (req, res) => {
	try {
		...
		try {
			...
			logger.error("Erreur lors de l'envoi de l'e-mail d'annulation :", error);
		}
	} catch (error) {
